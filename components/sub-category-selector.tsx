...
import { searchSubCategories } from 'actions/sub-category.action';
...
export const SubCategorySelector: React.FC<Props> = (props) => {
  const { name, control, rules, placeHolder = 'Choose Sub Category', containerClassName, onChange, isClearable = false } = props;
  ...
  const options = useDebouncedCallback(
    useCallback((inputValue: string, callback: (options: ICategory[]) => void) => {
      new Promise(async (resolve) => {
        const res = await searchSubCategories(inputValue);
        callback(res ?? []);
